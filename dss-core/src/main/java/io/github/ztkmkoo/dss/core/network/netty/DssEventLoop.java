...
public class DssEventLoop extends SingleThreadEventLoop {

    private static final InternalLogger logger = InternalLoggerFactory.getInstance(DssEventLoop.class);
    ...
    protected DssEventLoop(EventLoopGroup parent, Executor executor, SelectStrategyFactory selectStrategyFactory, Queue<Runnable> taskQueue, Queue<Runnable> tailTaskQueue, RejectedExecutionHandler rejectedExecutionHandler) {
        super(parent, executor, false, taskQueue, tailTaskQueue, rejectedExecutionHandler);
        this.selectStrategy = ObjectUtil.checkNotNull(selectStrategyFactory.newSelectStrategy(), "selectStrategy");
    ...
    private void closeAll() {
        ...
        Collection<DssServerSocketChannel> channels = new ArrayList<>(keys.size());
        for (SelectionKey k: keys) {
            Object a = k.attachment();
            if (a instanceof DssServerSocketChannel) {
                channels.add((DssServerSocketChannel) a);
            } else {
                k.cancel();
                @SuppressWarnings("unchecked")
                NioTask<SelectableChannel> task = (NioTask<SelectableChannel>) a;
        ...
        for (DssServerSocketChannel ch: channels) {
